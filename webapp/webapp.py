...
r'''
Created by Team-Sushan <Team-Sushan@gmail.com>
...
THREADS = int(os.environ.get('THREADS', sushasan.THREAD_COUNT_DEFAULT))
...
class Session():
	def __init__(self, url, nameservers=None, thread_count=THREADS):
		...
		self.url = sushasan.UrlParser(url)
		self.nameservers = nameservers
		self.thread_count = thread_count
		self.jobs = Queue()
		self.threads = []
		self.fuzzer = sushasan.Fuzzer(self.url.domain, dictionary=DICTIONARY, tld_dictionary=TLD_DICTIONARY)
	...
	def scan(self):
		...
		for _ in range(self.thread_count):
			worker = sushasan.Scanner(self.jobs)
			worker.option_extdns = sushasan.MODULE_DNSPYTHON
			worker.option_geoip = sushasan.MODULE_GEOIP
			if self.nameservers:
	...
	def csv(self):
		return sushasan.Format(self.permutations(registered=True)).csv()

	def json(self):
		return sushasan.Format(self.permutations(registered=True)).json()

	def list(self):
		return sushasan.Format(self.permutations()).list()
...
def api_scan():
	...
	try:
		_, domain, _ = sushasan.domain_tld(j.get('url'))
	except Exception:
		return jsonify({'message': 'Bad request'}), 400
...
def api_csv(sid):
	for s in sessions:
		if s.id == sid:
			return s.csv(), 200, {'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=sushasan.csv'}
	return jsonify({'message': 'Scan session not found'}), 404
...
def api_json(sid):
	for s in sessions:
		if s.id == sid:
			return s.json(), 200, {'Content-Type': 'application/json', 'Content-Disposition': 'attachment; filename=sushasan.json'}
	return jsonify({'message': 'Scan session not found'}), 404
...
def api_list(sid):
	for s in sessions:
		if s.id == sid:
			return s.list(), 200, {'Content-Type': 'text/plain', 'Content-Disposition': 'attachment; filename=sushasan.txt'}
	return jsonify({'message': 'Scan session not found'}), 404
