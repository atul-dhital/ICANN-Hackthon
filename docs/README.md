...
In a hurry? Try it in your web browser: [sushasan.it](https://sushasan.it)
...
```
$ git clone https://github.com/elceef/sushasan.git
$ cd sushasan
$ pip install .
```
...
```
$ sudo apt install sushasan
```

**Fedora Linux**

```
$ sudo dnf install sushasan
```
...
This will install `sushasan` along with all dependencies, and the binary will
be added to `$PATH`.

```
$ brew install sushasan
```
...
```
$ sushasan --registered domain.name
```
...
please supply `sushasan` with a dictionary file. Some dictionary samples with
a list of the most common words used in phishing campaigns are included.

```
$ sushasan --dictionary dictionaries/english.dict domain.name
```
...
```
$ sushasan --tld dictionaries/common_tlds.dict domain.name
```
...
```
$ sushasan --fuzzers "homoglyph,hyphenation" domain.name
```
...
```
$ sushasan --format csv domain.name | column -t -s,
$ sushasan --format json domain.name | jq
```
...
```
$ sushasan --format list domain.name
```
...
time-consuming. To address this, `sushasan` makes use of so-called fuzzy hashes
...
`--lsh` argument. For each generated domain, `sushasan` will fetch content
...
```
$ sushasan --lsh domain.name
```

In some cases, phishing sites are served from a specific URL. If you provide a
full or partial URL address as an argument, `sushasan` will parse it and apply
for each generated domain name variant. Use `--lsh-url` to override URL to
fetch the original web page from.

```
$ sushasan --lsh https://domain.name/owa/
$ sushasan --lsh --lsh-url https://different.domain/owa/ domain.name
```
...
```
$ sushasan --lsh tlsh domain.name
```

**Perceptual hashing**

If Chromium browser is installed, `sushasan` can utilize its headless mode,
...
```
$ sushasan --phash --screenshots /tmp/domain domain.name
```
...
```
>>> import sushasan
>>> data = sushasan.run(domain='domain.name', registered=True, format='null')
```
...
```
>>> sushasan.run(domain='domain.name', format='list', output=sushasan.devnull)
```

The arguments for `sushasan.run()` are translated internally, so the usage is
very similar to the command line. The returned data structure is an
easy-to-process list of dictionaries. Keep in mind that `sushasan.run()` spawns
